...
export default function SimilarMovies({ similar }: SimilarMoviesProps) {
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold text-fg mb-4">비슷한 영화</h2>
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 md:gap-4">
        {similar.slice(0, 10).map((m) => (
          <Link key={m.id} href={`/movies/${m.id}`} className="group">
            <div className="relative aspect-[2/3] rounded-md overflow-hidden bg-surface-card">
              {m.poster_path ? (
                <Image
            ...
            <div className="mt-1.5 text-center">
              <p className="text-sm font-medium text-fg group-hover:text-primary-400 transition truncate">
                {m.title_ko || m.title}
              </p>
              {m.genres.length > 0 && (
