  ...
  return (
    <>
      ...
      {isSemanticMode && (isSemanticLoading || semanticResults.length > 0) && (
        <div>
          <div className="px-4 py-2 bg-gradient-to-r from-purple-900/30 to-surface/50 flex items-center space-x-2">
          ...
          {isSemanticLoading ? (
            <div className="px-4 py-3 flex items-center space-x-3">
              ...
              <span className="text-fg/50 text-sm">AI 추천 검색 중...</span>
            </div>
          ) : (
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-4 py-3 transition text-left ${
                    isActive ? "bg-overlay/10" : "hover:bg-overlay/5"
                  }`}
                >
                  <div className="relative w-10 h-14 flex-shrink-0 bg-surface rounded overflow-hidden">
                    {movie.poster_path ? (
                      <Image
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Film className="w-5 h-5 text-fg/20" />
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-fg/50 text-sm">
                      ...
                      {movie.genres.length > 0 && (
                        <span className="text-fg/30 truncate">
      ...
      {results && results.movies.length > 0 && (
        <div>
          <div className="px-4 py-2 bg-surface/50 flex items-center space-x-2">
            <Film className="w-4 h-4 text-primary-500" />
            <span className="text-xs font-medium text-fg/60 uppercase tracking-wide">
        ...
        <div>
          ...
          {results.people.map((person) => {
            ...
            return (
              ...
              >
                <div className="w-10 h-10 flex-shrink-0 bg-surface rounded-full flex items-center justify-center">
                  <User className="w-5 h-5 text-fg/30" />
                </div>
                <p className="text-fg">
      ...
      {results &&
        ...
        !isSemanticLoading && (
          <div className="p-6 text-center">
            <Search className="w-8 h-8 text-fg/20 mx-auto mb-2" />
            <p className="text-fg/60">검색 결과가 없습니다</p>
            <p className="text-fg/40 text-sm mt-1">다른 키워드로 검색해보세요</p>
          </div>
        )}
